#!/usr/bin/env bash
# Runner: OpenAI Codex CLI (template — verify flags against your installed version).
# Args: $1 = repo dir, $2 = prompt file
set -euo pipefail
cd "$1"

# Codex has no /goal or /bootstrap skill, so both workflows are spelled out here.
...
# --model here is a common convention across CLIs, not confirmed for Codex —
# check `codex exec --help` and adjust the flag name if needed.
MODEL_FLAGS=()
[ -n "${MODEL:-}" ] && MODEL_FLAGS+=(--model "$MODEL")

exec codex exec --full-auto "${MODEL_FLAGS[@]}" "$TASK"
